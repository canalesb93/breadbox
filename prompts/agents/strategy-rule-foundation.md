...
5. **Create the vetted rules** (`create_transaction_rule` — pass a `rules` array for several at once). Rules write the category source as `none` and must NEVER overwrite a user-locked category (`category_override='user'` is sacred).
