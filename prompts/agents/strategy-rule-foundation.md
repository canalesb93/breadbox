...
5. **Create the vetted rules** (`create_transaction_rule`, or `batch_create_rules` for several at once). Rules write `category_id` directly (last-writer-wins; no provenance). Rules only run on new or changed transactions, so they won't continuously re-clobber a category a human edited on an otherwise-unchanged row.
