...
1. Call `get_overview` for context. `list_categories` for the taxonomy. `list_transaction_rules` to see what already exists — improve coverage and fill gaps; never duplicate an existing rule.
...
5. **Create the vetted rules** (`create_transaction_rule` — pass a `rules` array for several at once). Rules write `category_id` directly (last-writer-wins; no provenance). Rules only run on new or changed transactions, so they won't continuously re-clobber a category a human edited on an otherwise-unchanged row.
