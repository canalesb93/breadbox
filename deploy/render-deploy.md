...
- **breadbox** — pulled from `ghcr.io/canalesb93/breadbox:latest`
  (the prebuilt multi-arch image, **no Dockerfile build at deploy time**),
...
- Connecting your Render account (sign up if needed — free, no card
...
**not** auto-redeploy when upstream `main` advances — that's by
design (most self-hosters of a financial app want to control upgrade
timing).
...
when a newer GitHub release exists, so you'll know when there's
something new to pull.

To pin to a specific release, edit the `image.url` in `render.yaml`
on your fork (or your own template copy) — e.g.
