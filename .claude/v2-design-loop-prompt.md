...
3. **Load the design skills.** Invoke the `frontend-design` skill (use `Skill` with `skill: "frontend-design"`) for the design vision. For shadcn primitives, use the **shadcn MCP tools directly** (`mcp__shadcn__search_items_in_registries`, `mcp__shadcn__view_items_in_registries`, `mcp__shadcn__get_add_command_for_items`, `mcp__shadcn__get_audit_checklist`, `mcp__shadcn__list_items_in_registries`, `mcp__shadcn__get_item_examples_from_registries`, `mcp__shadcn__get_project_registries`). There is a dangling `.claude/skills/shadcn` symlink in the repo — do not try to invoke it as a Skill, it will fail. Check what's already in `web/src/components/ui/` (run `ls web/src/components/ui`) before adding new primitives. When adding, prefer the `new-york` style components matching `web/components.json`.
