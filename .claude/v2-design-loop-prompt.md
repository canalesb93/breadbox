...
3. **Load the design skills.** Invoke the `frontend-design` skill (use `Skill` with `skill: "frontend-design"`) for the design vision and the shadcn skill / shadcn MCP tools (`mcp__shadcn__*`) to find and install any shadcn primitives you need that aren't already in `web/src/components/ui/` (check first with `ls web/src/components/ui`). When adding new primitives, prefer the `new-york` style components that match `web/components.json`.
