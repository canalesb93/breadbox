...
- **PR 5 — split into 5a/5b/5c:**
  ...
  - [ ] **PR 5b — configure drawer (`drawer drawer-end`).** Mintlify-style right slide-over: per-preset trigger (schedule/event), schedule presets + cron, apply-mode/behavior, additional instructions, projected cost. Save & enable.
...
**Screenshot recipe (works in the autonomous loop):** build `bin/breadbox`; `create-admin --username admin --password breadbox123` on `breadbox_test_wf`; serve w/ `BREADBOX_DEV_RELOAD=1 SERVER_PORT=8092 ENCRYPTION_KEY=<64hex>` (bg); `npm i playwright-core` in job tmp + a script using `chromium.launch({channel:'chrome',headless:true})` — login via curl to get the HttpOnly `session` cookie, inject via `ctx.addCookies`, goto the page, screenshot JPEG; upload `curl -X POST https://img402.dev/api/free -F image=@shot.jpg` → `.url`. (Browser MCPs need interactive approval / have a locked profile in the bg loop.)
