...
- [ ] **PR 2b — agent precedence behavior.** Branch `runUpdateOpInTx`/`SetTransactionCategory` on `actor.Type=='agent'` → guarded `<>'user'` write stamping `'agent'`; non-agent → `'user'`. Report user-locked skips as `Status:"skipped"` (+ counter in handleUpdateTransactions). New tests: agent write fills 'none' & overwrites 'agent' but SKIPS 'user'; user write overwrites agent. Update MCP `update_transactions` description for the agent stamp.
