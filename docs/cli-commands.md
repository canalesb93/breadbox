...
| `breadbox agent run <slug> [--json] [--prefix "..."]` | L | Trigger an immediate run of the named agent — same path as the v2 SPA "Run now" button. Mints a scoped API key, spawns the sidecar, persists the agent_runs row, revokes the key. `--prefix` prepends a per-run operator note to the prompt for this fire only. `--json` switches to the full AgentRunResponse JSON |
| `breadbox agent list [--json]` | L | Print every agent definition with status, schedule, model, and next-fire time as a table. `--json` emits the full AgentDefinitionResponse array for piping into jq. Useful for SSH-based ops without the v2 SPA |
