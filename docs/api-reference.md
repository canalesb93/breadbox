...
Re-importing the same CSV is a safe no-op — every row hashes to the same `provider_transaction_id` and the upsert detects the dedup. **A transaction's category is preserved** across re-imports — raw re-sync never touches `category_id` (it is enrichment owned by rules/agents/users).
...
| `category_slug` | string | Optional. Sets `category_id` (last-writer-wins; no provenance). Mutually exclusive with `reset_category`. |
| `reset_category` | bool | Optional. Drops the transaction back to `uncategorized` so rules can re-categorize. |
...
| POST | `/rules/apply-all` | Write | Apply all active rules retroactively (pipeline-stage order) |
