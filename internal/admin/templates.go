...
func NewTemplateRenderer(sm *scs.SessionManager) (*TemplateRenderer, error) {
	tr := &TemplateRenderer{
		...
		sm:        sm,
		// funcMap is intentionally minimal: most admin pages render via
		// templ components which compile their own helpers. The few entries
		// here are the only helpers still referenced by the surviving
		// html/template pages (agents, mcp_settings, oauth_*, connection_reauth)
		// and the partial bridge in renderComponent. See issue #462 for the
		// migration that removed the rest.
		funcMap: template.FuncMap{
