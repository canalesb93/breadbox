...
// --- CountPendingReviews (sqlc query) excludes matched dependent transactions ---

func TestCountPendingReviews_ExcludesMatchedDependentTransactions(t *testing.T) {
	svc, queries, _ := newService(t)
	ctx := context.Background()

	// Create two users with separate connections (primary + dependent scenario).
	userPrimary := testutil.MustCreateUser(t, queries, "PrimaryUser")
	userDependent := testutil.MustCreateUser(t, queries, "DependentUser")

	connPrimary := testutil.MustCreateConnection(t, queries, userPrimary.ID, "item_cpd_primary")
	connDependent := testutil.MustCreateConnection(t, queries, userDependent.ID, "item_cpd_dependent")

	acctPrimary := testutil.MustCreateAccount(t, queries, connPrimary.ID, "ext_cpd_primary", "Primary Checking")
	acctDependent := testutil.MustCreateAccount(t, queries, connDependent.ID, "ext_cpd_dependent", "Dependent Card")

	// Create matching transactions on both accounts.
	txnPrimary := testutil.MustCreateTransaction(t, queries, acctPrimary.ID, "txn_cpd_p1", "Grocery Store", 5000, "2025-03-01")
	txnDependent := testutil.MustCreateTransaction(t, queries, acctDependent.ID, "txn_cpd_d1", "Grocery Store", 5000, "2025-03-01")

	// Also create a normal (non-linked) transaction with a review.
	txnNormal := testutil.MustCreateTransaction(t, queries, acctPrimary.ID, "txn_cpd_normal", "Coffee", 350, "2025-03-02")

	// Enqueue reviews for all three transactions.
	mustEnqueueReview(t, queries, txnPrimary.ID, "new_transaction")
	mustEnqueueReview(t, queries, txnDependent.ID, "new_transaction")
	mustEnqueueReview(t, queries, txnNormal.ID, "new_transaction")

	// Before linking: all 3 should be counted.
	rawCount, err := queries.CountPendingReviews(ctx)
	if err != nil {
		t.Fatalf("CountPendingReviews before link: %v", err)
	}
	if rawCount != 3 {
		t.Errorf("before link: expected 3 pending, got %d", rawCount)
	}

	// Create account link and mark dependent as linked.
	link, err := queries.CreateAccountLink(ctx, db.CreateAccountLinkParams{
		PrimaryAccountID:   acctPrimary.ID,
		DependentAccountID: acctDependent.ID,
	...
	err = queries.UpdateAccountDependentLinked(ctx, db.UpdateAccountDependentLinkedParams{
		ID:                acctDependent.ID,
		IsDependentLinked: true,
	})
	if err != nil {
		t.Fatalf("UpdateAccountDependentLinked: %v", err)
	}

	// Create a transaction match between the two transactions.
	_, err = queries.CreateTransactionMatch(ctx, db.CreateTransactionMatchParams{
		AccountLinkID:          link.ID,
		PrimaryTransactionID:   txnPrimary.ID,
		DependentTransactionID: txnDependent.ID,
		MatchConfidence:        "auto",
		MatchedOn:              []string{"date_amount_name"},
	})
	if err != nil {
		t.Fatalf("CreateTransactionMatch: %v", err)
	}

	// After linking + matching: the dependent's review should be excluded.
	// CountPendingReviews (sqlc) should return 2 (primary + normal), not 3.
	rawCount, err = queries.CountPendingReviews(ctx)
	if err != nil {
		t.Fatalf("CountPendingReviews after match: %v", err)
	}
	if rawCount != 2 {
		t.Errorf("after match: CountPendingReviews expected 2, got %d", rawCount)
	}

	// GetReviewCounts (service) should agree.
	counts, err := svc.GetReviewCounts(ctx)
	if err != nil {
		t.Fatalf("GetReviewCounts: %v", err)
	}
	if counts.Pending != 2 {
		t.Errorf("after match: GetReviewCounts.Pending expected 2, got %d", counts.Pending)
	}
}
