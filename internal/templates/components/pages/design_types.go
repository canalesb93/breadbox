...
func DesignSections() []DesignSection {
	return []DesignSection{
		...
		},
		{
			...
			Description: "Shared chip for the Recurring (recurring-series) surfaces: SeriesTypeChip (structured type). Used on the ledger rows and the detail header. A thin, rule-maintained series has no detector-derived renewal/confidence state, so the type chip is the only series chip.",
			Group:       "data",
		...
		},
		{
			...
			Description: "The /recurring series ledger as daisy list-rows inside an AgentRunRowList card: a leading type glyph, the name + type chip, and the linked-charge count. A thin, rule-maintained series carries no status/renewal/cadence/amount — the whole row links to the detail page. Same list-row shape as /accounts and /rules.",
			Group:       "data",
		...
		},
		{
			...
			Description: "GoverningRulesPanel for the recurring-series detail page — the rules-as-substrate payoff: a series IS the rules that define it. Lists the assign_series rules whose matches populate the series (name + condition summary + link to the rule editor + lifetime hit count), with an empty state that explains the model. The handler assembles the values; this is pure presentation.",
			Group:       "data",
