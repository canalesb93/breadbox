...
- **Attribution-aware filtering**: transaction queries resolve the owning user via `COALESCE(t.attributed_user_id, a.owner_user_id, bc.user_id)` — per-transaction override (account links) **>** per-account owner override (`accounts.owner_user_id`) **>** connection owner. Resolved at read time, so reassigning an account's owner re-routes its existing transactions with no backfill. Dependent-linked accounts excluded from totals via `a.is_dependent_linked = FALSE`.
