...
.PHONY: dev dev-watch dev-stop build test test-integration lint generate migrate-up migrate-down migrate-create sqlc sqlc-install seed db db-stop docker-up docker-down css css-watch css-install air-install templ-install templ

PORT ?= 8080

# generate ensures gitignored build artifacts exist.
# Skips if artifacts are already present (e.g., copied by .worktreeinclude).
# Run 'make sqlc' or 'make css' or 'make templ' directly to force regeneration.
generate:
